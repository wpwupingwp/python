...
def parse_args():
    ...
    arg.add_argument("-o", "-out", dest="out", help="output prefix", default="draw_out")
    ...
    option.add_argument("-horizon", action="store_false", help="figure direction")
    option.add_argument("-italic", action="store_true", help="italic labels")
    option.add_argument(
        "-merge_small", action="store_true", help="merge small values in pie"
    )
    ...
    option.add_argument(
        "-y_percent", action="store_true", help="set y ticks to percent"
    )
...
def boxplot(arg):
    ...
    4,5,6
    """
    flierprops = dict(marker="o", markeredgewidth=0.5, markersize=5, alpha=0.5)
...
def pieplot(arg):
    ...
    if arg.italic:
        labels = [f"${{{i}}}$" for i in labels]
    pie = plt.pie(
        count, labels=labels, autopct=lambda pct: func(pct, count), colors=colors
    )
    svg2emf(plt, arg)
    return arg.out
...
def stackplot(arg):
    ...
    filtered_data = [raw_data[i].dropna() for i in raw_data]
    sp = plt.stackplot(
        range(len(filtered_data[0])), filtered_data, labels=labels, colors=colors
    )
...
def stackplot2(arg):
    ...
    for i in range(1, len(filtered_data)):
        p1 = plt.bar(types, filtered_data[i], label=labels[i])
    # for patch, color in zip(sp['stacks'], colors):
    #    patch.set_facecolor(color)
    plot_set(plt, arg)
    plt.legend(loc="upper left")
