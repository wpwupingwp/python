...
def count(alignment, rows, columns):
    ...
    for index in range(columns):
        ...
        count_dict = {b"A": 0, b"T": 0, b"C": 0, b"G": 0, b"N": 0, b"-": 0, b"?": 0}
...
def validate(candidate_file, input_file, n_seqs, min_len, min_covrage, max_mismatch):
    ...
    for query in SearchIO.parse(blast_result_file, "blast-xml"):
        ...
        blast_result.append(
            [query.id, good_hits / n_seqs, sum_bitscore_raw, start / n_seqs]
        )
    # validate
...
def parse_args():
    ...
    arg.add_argument(
        "-c", "--cutoff", type=float, default=0.95, help="minium percent to keep"
    )
    arg.add_argument(
        "-g",
        "--gap_cutoff",
    ...
    arg.add_argument(
        "-lmin", "--min_len", type=int, default=24, help="minimum primer length range"
    )
    arg.add_argument(
        "-lmax", "--max_len", type=int, default=25, help="maximum primer length range"
    )
    arg.add_argument(
        "-m", "--mismatch", type=int, default=2, help="maximum mismatch bases in primer"
    )
    # arg.print_help()
...
def main():
    ...
    primer_candidate = find_primer(
        continuous, most, arg.min_len, arg.max_len, arg.ambiguous_base_n
    )
    candidate_file = write_fastq(
        primer_candidate, rows, arg.name + ".candidate.fastq", arg.name
    )
    primer_info = validate(
        candidate_file, arg.input, rows, arg.min_len, arg.cutoff, arg.mismatch
    )
