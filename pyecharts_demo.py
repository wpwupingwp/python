...
bar = Bar(
    init_opts=options.InitOpts(
        width="1500px", height="600px", page_title="Demo", theme=ThemeType.DARK
    )
)
bar.add_xaxis(city)
...
bar.set_global_opts(
    title_opts=options.TitleOpts(title=province + "20200206"),
    datazoom_opts=options.DataZoomOpts(type_="inside"),
    toolbox_opts=options.ToolboxOpts(),
    visualmap_opts=options.VisualMapOpts(
        max_=zuidazhi, split_number=10, is_piecewise=True
    ),
)
bar.set_series_opts(
