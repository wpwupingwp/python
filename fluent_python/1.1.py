...
class Card:
    ...
    def __unicode__(self):
        ...
        if self == Card('White', 'joker'):
            ...
            pass
        _ranks_u = ['A', ] + [str(n) for n in range(2, 11)] + list('JQK')
...
class Deck:
    _ranks = [str(n) for n in range(3, 11)] + list('JQKA') + ['2', ]
    _suits = 'diamonds clubs hearts spades'.split()

    # card_n = rank_n + 1 + suit_n * 13
    def __init__(self, joker=False):
        self.raw_cards = [Card(rank, suit) for rank in self._ranks for suit in
                          self._suits]
        if joker:
            self.raw_cards.extend([Card('red', 'joker'),
                                   Card('white', 'joker')])
