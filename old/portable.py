    ...
    for i in raw_seq.features:
        if i.type == "gene" and "gene" in i.qualifiers:
            if i.location_operator != "join":
                ...
                sequence = "".join(
                    [str(raw_seq.seq[begin:end]), str(raw_seq.seq[begin:end])]
                )
                rec = [
                    taxon_id,
                    organism,
            ...
            if "number" in i.qualifiers:
                name = "{0}_exon_{1}".format(
                    i.qualifiers["gene"][0], i.qualifiers["number"][0]
                )
            else:
...
def update_seq_db_from_genbank(date):
    ...
    user_email = input(
        "Input your email address for downloading data or use default(by press enter):\n"
    )
    if user_email is "\n":
    ...
    w = handle["WebEnv"]
    k = handle["QueryKey"]
    genome_content = Entrez.efetch(
        db="nuccore", webenv=w, query_key=k, rettype="gb", retmode="text"
    )
...
def init_taxon():
    ...
    for line in taxon:
        ...
        cur.execute(
            "INSERT INTO taxon (Id, Name, Rank, Son, Parent, GreatSon) VALUES (?, ?, ?, ?, ?, ?);",
