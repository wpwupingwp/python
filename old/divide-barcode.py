    ...
    for record in SeqIO.parse(arg.input, "fastq"):
        if record.seq[:8] in barcode:
            filename = os.path.join(
                arg.out,
                "{0}-{1}.fastq".format(
                    os.path.splitext(arg.input)[0], barcode[record.seq[:8]]
                ),
            )
...
def main():
    ...
    parser.add_argument(
        "-b", dest="barcode_file", help="csv file containing barcode info"
    )
    ...
    end_time = timer()
    print(
        "Finished with {0:.3f}s. You can find results in {1}.\n".format(
